...
type SubmitData = Omit<FormData, "achievementType"> & {
  achievementType: AchievementType;
};
...
export default function ScholarAchievementForm({
  ...
  function validate(): boolean {
    ...
    if (!form.proofLink.trim()) {
      e.proofLink = "Please provide a link to your proof of achievement.";
    } else {
      try { new URL(form.proofLink.trim()); } catch {
        e.proofLink = "Please enter a valid URL (e.g. https://drive.google.com/…).";
      }
    }
    setErrors(e);
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!validate()) return;
    await onSubmit({ ...form, proofLink: form.proofLink.trim() } as SubmitData);
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="border-t border-white/[0.06] pt-2">
        ...
        <p className="text-xs text-slate-600 mt-1.5">
          Google Drive, Dropbox, or any publicly accessible link to your proof document.
        </p>
      </div>
