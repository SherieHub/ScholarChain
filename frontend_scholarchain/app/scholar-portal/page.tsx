            ...
            <div className="flex flex-col items-center gap-1">
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border transition-colors ${
                  ...
                  current ? "bg-blue-500/20  border-blue-500/60  text-blue-300"     :
                            "bg-white/[0.03] border-white/[0.10] text-slate-600"
                }`}
              >
                {done ? "✓" : n}
              </div>
              <span className={`text-xs ${current ? "text-white" : done ? "text-green-400" : "text-slate-600"}`}>
                {label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={`flex-1 h-px mx-2 mb-4 ${done ? "bg-green-500/40" : "bg-white/[0.08]"}`} />
            )}
          </div>
        );
      })}
...
function PortalContent() {
  ...
  const handleSubmitAchievement = async (data: { subject: string; grade: string; proofLink: string }) => {
    ...
    try { await submitAchievement(scholar.id, data); }
    finally { setIsSubmittingAchievement(false); }
  };
        ...
        <div className="flex flex-col items-center gap-6 py-10 px-4 text-center bg-white/[0.03] border border-white/[0.08] rounded-2xl">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              Your wallet will ask you to sign a short message to prove ownership.
              No ADA is spent — this is a read-only authentication step.
            </p>
          </div>
          ...
          <button onClick={handleDisconnect} className="text-sm text-slate-500 hover:text-slate-300 underline transition-colors">
      ...
      <>
        ...
        <div className="flex flex-col items-center gap-6 py-10 px-4 text-center bg-white/[0.03] border border-white/[0.08] rounded-2xl">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              You closed the signing window before approving. Sign the message to continue — no ADA is spent.
            </p>
          </div>
  ...
  if (portalState === "authorized" && !scholar) {
    return (
      <div className="flex flex-col items-center gap-6 py-10 px-4 text-center bg-white/[0.03] border border-white/[0.08] rounded-2xl">
        ...
        <button onClick={handleDisconnect} className="text-sm text-slate-400 hover:text-white underline transition-colors">
